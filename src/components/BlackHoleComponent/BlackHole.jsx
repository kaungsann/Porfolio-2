...
function BlackHole({ removeHole }) {
  ...
  useEffect(() => {
    if (!removeHole) {
      setTimeout(() => setIsVisible(false), 1000); // Match this duration with your transition time
    } else {
      setIsVisible(true);
    }
  }, [removeHole]);
  return (
    <>
      {isVisible && (
        <div
          className={`bg-[#050d24] overflow-hidden opacity-95 fixed top-0 left-0 w-full h-screen right-0 z-30 bg-transition ${
            !removeHole ? "hide" : ""
          }`}
        >
          <main className="relative circleBox">
            ...
            >
              ...
              >
                <a
                  href="https://kaungsann.github.io/my-portfolio/"
                  className=""
                >
                  <img
                    src={portfolio}
